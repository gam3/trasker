...
char *getatom(Window target_win, Atom atom, long *length, Atom *type, int *size);
...
int
get_desktops(char **strings, int max)
{
    Atom atom;
    Atom type;
    int  size;
	...
	if (a) {
	    Atom xatom = XInternAtom(my_display, "_WIN_WORKSPACE_NAMES", 0);
...
test_focus(struct window_info *win_info, XScreenSaverInfo *mit_info, long idletime)
{
	...
	long length;
	long *c;
	c = (long *)getatom(DefaultRootWindow(my_display), atom, &length, &type, &size);
	if (c) {
	    win_info->desktop = *c;
	    XFree(c);
	} else {
...
fprintf(stderr, "active\n");
    ...
    if (terminal_status.v_active != our_virtual_terminal) {
	static char buf[10];
	win_info->window = 0;
    ...
    long *length, Atom *type, int *size)
{
    ...
    status = XGetWindowProperty(my_display, target_win, atom, 0, (max_len+3)/4,
...
main(int argc, char *argv[])
{
    ...
    tty0_file_descriptor = open("/dev/tty0", O_RDONLY);
	...
	if (current_virtual_terminal == -1)
	    printf("An error occurred while executing the ioctl. No information was supplied.\n");
	current_virtual_terminal = -3;
    }
    if (current_virtual_terminal == -1) {
	...
	if (!log_file) {
	    sprintf(buffer, "%s/.tasker.%s.%d.%s", home, host, atoi(port), "log");
	    log_file = strdup(buffer);
	}
	if (!pid_file) {
	    sprintf(buffer, "%s/.tasker.%s.%d.%s", home, host, atoi(port), "pid");
	...
	if (buffer[0] == ':') {
	    ...
	    strncat(buffer, XDisplayString(my_display), 80 - strlen(buffer));
	}
        buffer[80] = '\0';
	{
    ...
    tty_file_descriptor = open("/dev/tty", O_RDWR);
...
void
log_window(FILE *log, struct window_info *current_window_info)
{
    if (log) {
...
void
rdf_window(FILE *frdf, struct window_info *current_window_info, char **desktops)
{
	...
	if (current_window_info->start_time) {
	    if (verbose)
		fprintf(stderr, "tracker\t%s\t%s\t%d\t%s\t%lu\t%s\t%s\t%s\t%s\t%s\n",
		    user,
		    host_name,
		    current_window_info->idle,
	    ...
	    fprintf(frdf, "tracker\t%s\t%s\t%d\t%s\t%lu\t%s\t%s\t%s\t%s\t%s\n",
		user,
		host_name,
		current_window_info->idle,
		desktops[current_window_info->desktop],
...
void
rdf_xwindow(FILE *frdf, struct window_info *current_window_info)
{
    if (frdf) {
	if (current_window_info->start_time) {
	    if (verbose > 1)
...
void
loop(int rfd, long idletime, char **desktops, int desktopcnt)
{
    ...
    memmove(&current_window_info, &win_info[x], sizeof(struct window_info));
...
int
get_remotefd(char *server_addr_s, int port)
{
